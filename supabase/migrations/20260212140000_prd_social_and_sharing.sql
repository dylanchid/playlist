-- PRD: context story, sharing, reactions, friend activity, profile prefs.
-- Aligns with fix-database-prd-alignment.sql; fixes public shares (shared_with NULL) and idempotent policies.

-- 1. Playlists: context story
...
-- 2. Shares: shared_with nullable for share_type = 'public'
...
-- Existing DBs: drop NOT NULL / old unique if present
ALTER TABLE public.playlist_shares
  ALTER COLUMN shared_with DROP NOT NULL;

ALTER TABLE public.playlist_shares
  DROP CONSTRAINT IF EXISTS playlist_shares_playlist_id_shared_by_shared_with_key;

CREATE UNIQUE INDEX IF NOT EXISTS idx_playlist_shares_friend_target
  ON public.playlist_shares (playlist_id, shared_by, shared_with)
  WHERE shared_with IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_playlist_shares_public_one
  ON public.playlist_shares (playlist_id, shared_by)
  WHERE share_type = 'public' AND shared_with IS NULL;
...
-- 4. user_follows compatibility columns (skip if table absent)
DO $$
BEGIN
  IF EXISTS (
...
-- 5. user_profiles.music_preferences
ALTER TABLE public.user_profiles
ADD COLUMN IF NOT EXISTS music_preferences jsonb DEFAULT '{}';

-- 6. Friend activity (activity types match app FriendActivity)
CREATE TABLE IF NOT EXISTS public.friend_activities (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  activity_type varchar(30) NOT NULL CHECK (activity_type IN (
    'shared_playlist', 'reacted', 'commented', 'liked_playlist', 'followed_user',
    'playlist_create', 'follow', 'comment'
  )),
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE public.friend_activities DROP CONSTRAINT IF EXISTS friend_activities_activity_type_check;
    ALTER TABLE public.friend_activities ADD CONSTRAINT friend_activities_activity_type_check CHECK (activity_type IN (
      'shared_playlist', 'reacted', 'commented', 'liked_playlist', 'followed_user',
      'playlist_create', 'follow', 'comment'
    ));
  END IF;
...
-- 7. playlist_comments (optional table)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'playlist_comments'
  ) THEN
    ALTER TABLE public.playlist_comments ADD COLUMN IF NOT EXISTS responds_to_context boolean DEFAULT true;
    ALTER TABLE public.playlist_comments ADD COLUMN IF NOT EXISTS parent_comment_id uuid REFERENCES public.playlist_comments(id) ON DELETE CASCADE;
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Users can view playlist shares they're involved in" ON public.playlist_shares;
DROP POLICY IF EXISTS "Users can create playlist shares" ON public.playlist_shares;
DROP POLICY IF EXISTS "Users can delete their own playlist shares" ON public.playlist_shares;

CREATE POLICY "Users can view playlist shares they're involved in" ON public.playlist_shares
  FOR SELECT USING (
    shared_by = auth.uid()
    OR shared_with = auth.uid()
    OR (share_type = 'public' AND shared_with IS NULL)
  );
...
DROP POLICY IF EXISTS "Anyone can view playlist reactions" ON public.playlist_reactions;
DROP POLICY IF EXISTS "Users can manage their own reactions" ON public.playlist_reactions;
...
CREATE OR REPLACE FUNCTION public.calculate_music_compatibility(user1_id uuid, user2_id uuid)
RETURNS integer AS $$
DECLARE
  common_tags text[];
  user1_tags text[];
...
CREATE OR REPLACE FUNCTION public.update_compatibility_scores()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE public.user_follows
  SET compatibility_score = public.calculate_music_compatibility(follower_id, following_id)
...
BEGIN
  IF EXISTS (
    ...
    CREATE TRIGGER update_compatibility_on_playlist_change
      ...
      FOR EACH ROW
      EXECUTE FUNCTION public.update_compatibility_scores();
  END IF;
END $$;
